...
import no.vaccsca.amandman.backend.domain.util.NavdataUtils.getInterpolatedSpeedExpectation
import no.vaccsca.amandman.backend.domain.util.NavigationUtils.interpolatePositionAlongPath
import no.vaccsca.amandman.backend.domain.util.NavigationUtils.isBehind
import no.vaccsca.amandman.backend.domain.util.SpeedConversionUtils
import no.vaccsca.amandman.backend.domain.util.WeatherUtils
import no.vaccsca.amandman.backend.domain.util.WeatherUtils.interpolateWeatherAtAltitude
    ...
    fun calculateDescentTrajectory(
        ...
        trajectoryPoints +=
                TrajectoryPoint(
                    ...
                    heading = runwayInfo.trueHeading.roundToInt(),
                )
